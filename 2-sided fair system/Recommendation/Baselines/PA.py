...
'''
with open('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\24\\time_l.txt', 'w') as f:
    np.savetxt(f, time_l)

with open('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\24\\folder_l.txt', 'w') as f:
    np.savetxt(f, folder)
'''
...
data = list(range(0, 8))
random.shuffle(data)
loc=random.sample(range(0, 8*8), n)
...
for cd in range(1,n_i): 
 ...
 o=0
 for ak in prev_up:
  target_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\24\\' +str(ak) + '\\target_G.npy', allow_pickle=True).tolist()
  ...
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' +str(ak) + '\\res_G.npy', res_G)
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' +str(ak) + '\\target_G.npy', target_G)
 ...
 for hm in d:
  ...
  if hm==0:
   ...
   target_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' +str(up[hm]) + '/target_G.npy', allow_pickle=True).tolist()
   res_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' +str(up[hm]) + '/res_G.npy', allow_pickle=True).tolist()
   target_G = np.asarray(target_G, dtype=np.float32)
   target_G = target_G.reshape(576, 576)
  ...
  if com_reqs:
      ...
      wait_t.append(WT(res_list,com_reqs))
      s_m=0
      for i in wait_t:
        s_m=s_m+i[0]
      avg1=s_m/len(wait_t)
      ...
      u_c=(fare_cal(com_reqs)-dist_of_path(path,distances,dag,map[dest])*(0.202/1.55))   

      u_ci[cd]=u_ci[cd]+u_c
      utility[hm]=((r_a-1)*utility[hm] + (u_c/tim))/r_a  
  ...
  for vc in com_reqs:
   ...
   o_x,o_y=vc
   target_G[o_x][o_y]=0
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' +str(int(up[hm])) + '/res_G.npy', res_G)
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' +str(int(up[hm])) + '/target_G.npy', target_G)
 ...
 gini_coeff.append(gini(np.array(utility)))
 utility1=np.sort(utility)
 ten_per_d=math.floor(0.1*n)
 gini_coeff_ten.append(gini(np.array(utility1[0:ten_per_d])))

 twfive_per_d=math.floor(0.25*n)
 gini_coeff_twfive.append(gini(np.array(utility1[0:twfive_per_d])))

 fifty_per_d=math.floor(0.5*n)
 gini_coeff_fifty.append(gini(np.array(utility1[0:fifty_per_d])))

 sevfive_per_d=math.floor(0.75*n)
 gini_coeff_sevfive.append(gini(np.array(utility1[0:sevfive_per_d])))
 if cd>0:
  print("u_ci[cd],u_ci[cd-1],u_ci[cd-2]",u_ci[cd],u_ci[cd-1],u_ci[cd-2])
  print("loc_cou",loc_cou)

  if  u_ci[cd]==u_ci[cd-1] or u_ci[cd]==u_ci[cd-2]:
...
fd_s = [427, 944, 994, 438,  454, 1158, 273,  190, 1143, 1228, 568, 585, 253, 956, 980]
fd_s=[802]
...
print("utility per hour",sum(utility)/len(utility))
print("platforms utility",sum(u_ci)/len(u_ci))
print("wt",sum(wt)/len(wt))
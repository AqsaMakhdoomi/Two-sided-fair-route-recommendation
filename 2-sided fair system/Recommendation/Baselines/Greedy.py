...
res_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\0\\res_G.npy', allow_pickle=True).tolist()
target_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\0\\target_G.npy', allow_pickle=True).tolist()
res_D = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\0\\res_D.npy', allow_pickle=True).tolist()
target_D = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\0\\target_D.npy', allow_pickle=True).tolist()
...
'''for x in range(0,1247):
 time_instant=np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\' +str(x) +'/time.npy', allow_pickle=True).tolist()
...
with open('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\time_l.txt', 'r') as f:
    time_l = np.loadtxt(f)
with open('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\folder_l.txt', 'r') as f:
   folder = np.loadtxt(f)
...
res_G = res_G.reshape(99,99)

target_G = np.asarray(target_G, dtype=np.float32)
target_G = target_G.reshape(99,99)
...
data = list(range(0, 9))
random.shuffle(data)
loc=random.sample(range(0, 9*9), n)
...
def greedy(src, dag, distances, dest, sps):
    path = []
    path.append(src)
    ...
    while src != map[dest]:
        reqs = []
        coords = []
        ind = np.where(dag == src)
        i, j = ind  

        try:
            ind1 = i + 1, j

            x = alpha * sps[src1] - sps[dag[ind1]] - distances[i[0]][j[0]]  

            if x[0] >= 1:
                reqs.append(res_G[dag[ind][0]][dag[ind1][0]])
                coords.append((i[0] + 1, j[0]))
                distances[i[0] + 1][j[0]] = distances[i[0] + 1][j[0]] + 1
        except IndexError:
            lm = 1
        try:
            ind2 = i, j + 1

            x = alpha * sps[src1] - sps[dag[ind2]] - distances[i[0]][j[0]]  

            if x[0] >= 1:

                reqs.append(res_G[dag[ind][0]][dag[ind2][0]])

                coords.append((i[0], j[0] + 1))

                distances[i[0]][j[0] + 1] = distances[i[0]][j[0] + 1] + 1
        ...
        try:
            ind3 = i + 1, j + 1
            x = alpha * sps[src1] - sps[dag[ind3]] - distances[i[0]][j[0]]  

            if x[0] >= 1:
                reqs.append(res_G[dag[ind][0]][dag[ind3][0]])
                coords.append((i[0] + 1, j[0] + 1))
                distances[i[0] + 1][j[0] + 1] = distances[i[0] + 1][j[0] + 1] + 1
        except IndexError:
            lm = 1

        index = np.argsort(np.array(reqs))

        ind_rev = index[::-1]
        max_v = coords[ind_rev[0]]
...
reqs_matrix=np.zeros((99,99), dtype=np.float32)
taken_reqs_matrix= np.zeros((99,99), dtype=np.float32)  
...
loc_cou=0
loc=[]
loc1=random.sample(range(0, 90), n)
for i in loc1:
  loc.append(map1[i])
...
for cd in range(1, n_i):  
      ...
      target_G = np.load(
          'C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\' + str(fol) + '/target_G.npy', allow_pickle=True).tolist()
...
for cd in range(1,n_i): 
 o=0
 for ak in prev_up:
  target_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\' +str(ak) + '/target_G.npy', allow_pickle=True).tolist()
  res_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\' +str(ak) + '/res_G.npy', allow_pickle=True).tolist()
  target_G = np.asarray(target_G, dtype=np.float32)
  target_G = target_G.reshape(99,99)

  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\100\\' +str(ak) + '/res_G.npy', res_G)
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\100\\' +str(ak) + '/target_G.npy', target_G)
 ...
 try:
     fare2 = [i for i in fare if i > 0]
     print("fare is", sum(fare2) / len(fare2))
 except:
     aaa=1
 for hm in d:
  capacity=2
  ...
  if driver_i==0:
   target_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\' +str(up[hm]) + '/target_G.npy', allow_pickle=True).tolist()
   res_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\99\\' +str(up[hm]) + '/res_G.npy', allow_pickle=True).tolist()
   target_G = np.asarray(target_G, dtype=np.float32)
   target_G = target_G.reshape(99,99)
   ...
   target_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\100\\' +str(up[hm]) + '/target_G.npy', allow_pickle=True).tolist()
   res_G = np.load('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\100\\' +str(up[hm]) + '/res_G.npy', allow_pickle=True).tolist()
   target_G = np.asarray(target_G, dtype=np.float32)
   target_G = target_G.reshape(99,99)
  ...
  o=o+1
  sps=sp(dest)
  dg_a, arr, r, c, dest, capacity, src, path,gr = DAG(res_G, map1[src], capacity,gr)
  ...
  pat, distances, = greedy(src, dag, distances, dest, sps)
  ...
  print("path,path2,pat",path12,path2,pat[::-1])
  ...
  path=[]
  for izzz in res_list:
   path.append(map1[izzz])
  ...
  if com_reqs:
      ...
      fu_c=(fare_cal(com_reqs)-dist_of_path(path,distances,dag,map[dest])*(0.202/1.55))   
  ...
  for vc in com_reqs:
   ...
   o_x,o_y=vc
   target_G[o_x][o_y]=0
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\100\\' +str(up[hm]) + '/res_G.npy', res_G)
  np.save('C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\DC2kmfolder\\100\\' +str(up[hm]) + '/target_G.npy', target_G)
  print("saved")
 ...
 if cd>0:
  print("u_ci[cd],u_ci[cd-1],u_ci[cd-2]",fu_ci[cd],fu_ci[cd-1],fu_ci[cd-2])
  print("loc_cou",loc_cou)

  if  fu_ci[cd]==fu_ci[cd-1] or fu_ci[cd]==fu_ci[cd-2] :

       loc_cou=loc_cou+1
  if loc_cou>500:
     loc_x=random.sample(range(0, 23), n)
     loc_y=random.sample(range(0, 23), n)
     loc= list(zip(loc_x,loc_y))
     loc_cou=0
 print("iteration i",cd)
 elements, loc_sparsity_arr, x, y, xind, yind, loc_arr = loc_sparsity(reqs_matrix)

print(lorenzcurve(np.array(futility1)))
...
print(np.sum(taken_reqs_matrix))